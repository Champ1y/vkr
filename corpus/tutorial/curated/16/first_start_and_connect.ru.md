---
title: "Первый запуск PostgreSQL 16 и подключение через psql"
pg_version: "16"
language: "ru"
...
# Первый запуск PostgreSQL 16 и подключение через psql
...
Чтобы начать работу с PostgreSQL 16, проверь `psql --version`, подключись через `psql -U <role> -d <database>`, проверь `SELECT version()`, создай учебную базу через `CREATE DATABASE`, затем создай таблицу и выполни `INSERT`/`SELECT`.
