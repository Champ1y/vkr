---
title: "Проверка состояния репликации в PostgreSQL 16"
pg_version: "16"
language: "ru"
...
official_backing:
  ...
  - "https://www.postgresql.org/docs/16/logical-replication-monitoring.html"
...
# Проверка состояния репликации в PostgreSQL 16
...
Чтобы проверить logical replication в PostgreSQL 16, смотри `pg_subscription` и `pg_stat_subscription` на subscriber, `pg_replication_slots` и `pg_stat_replication` на publisher, затем сделай тестовый `INSERT` на publisher и `SELECT` на subscriber.
