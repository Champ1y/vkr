---
title: "Пошаговая настройка logical replication в PostgreSQL 16"
pg_version: "16"
language: "ru"
...
official_backing:
  ...
  - "https://www.postgresql.org/docs/16/logical-replication-config.html"
...
# Пошаговая настройка logical replication в PostgreSQL 16
...
Для logical replication в PostgreSQL 16 на publisher включают `wal_level=logical`, проверяют `max_replication_slots` и `max_wal_senders`, создают роль с `REPLICATION`, настраивают `pg_hba.conf`, создают `CREATE PUBLICATION`, а на subscriber — совместимую таблицу и `CREATE SUBSCRIPTION`.
