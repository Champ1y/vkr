---
title: "Первый запуск PostgreSQL 18 и подключение через psql"
pg_version: "18"
language: "ru"
...
official_backing:
  ...
  - "https://www.postgresql.org/docs/18/tutorial-createdb.html"
  - "https://www.postgresql.org/docs/18/tutorial-accessdb.html"
...
# Первый запуск PostgreSQL 18 и подключение через psql
...
Чтобы начать работу с PostgreSQL 18, проверь `psql --version`, подключись через `psql -U <role> -d <database>`, проверь `SELECT version()`, создай учебную базу через `CREATE DATABASE`, затем создай таблицу и выполни `INSERT`/`SELECT`.
