---
title: "Правила безопасной генерации ответов в tutorial-режиме PostgreSQL 18"
pg_version: "18"
language: "ru"
...
# Правила безопасной генерации ответов в tutorial-режиме PostgreSQL 18
...
- требуется формальный version guard по ветке PostgreSQL 18;
...
Version guard — обязательный фильтр: для вопросов по PostgreSQL 18 retrieval должен приоритизировать и подтверждать факты через ветку `/docs/18/`. Evidence guard — проверка, что ключевые утверждения в финальном ответе действительно опираются на найденные official фрагменты, а не только на похожесть embedding.
...
3. Включи version guard и отфильтруй контент под PostgreSQL 18.
...
2) Совпадает ли версия (ветка /docs/18/)?
...
```text
Пример fallback:
"Не вижу достаточного official подтверждения для версии 18. Уточните целевой параметр/сценарий, и я сверю по official docs."
```
